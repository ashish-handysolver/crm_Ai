                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-12">
                    <div className="space-y-6">
                      <div className="space-y-4">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Personal Info</label>
                        <div className="relative group/input">
                          <User className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within/input:text-indigo-400 transition-colors" size={18} />
                          <input
                            type="text"
                            value={displayName}
                            onChange={e => setDisplayName(e.target.value)}
                            placeholder="Full Name"
                            className="w-full bg-black/20 border border-white/10 rounded-2xl py-4 pl-16 pr-6 text-sm font-bold text-white placeholder:text-slate-500 focus:bg-black/40 focus:outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/20 transition-all shadow-inner"
                            required
                          />
                        </div>
                        ...
                        {!editingUserId && (
                          <>
                            <div className="relative group/input">
                              <Mail className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within/input:text-indigo-400 transition-colors" size={18} />
                              <input
                                type="email"
                                value={email}
                            ...
                            <div className="relative group/input">
                              <Lock className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within/input:text-indigo-400 transition-colors" size={18} />
                              <input
                                type="password"
                                value={password}
                      ...
                      {[
                        ...
                        <label key={r.id} className={`block p-5 rounded-2xl border-2 transition-all cursor-pointer relative overflow-hidden group ${newUserRole === r.id ? 'bg-indigo-500/20 border-indigo-500 shadow-lg shadow-indigo-500/10' : 'bg-black/20 border-white/10 hover:border-white/20'}`}>
                          ...
                          <div className="flex items-start gap-4 relative z-10">
                            <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 mt-1 transition-all ${newUserRole === r.id ? `border-indigo-500 bg-indigo-500` : 'border-slate-500'}`}>
                              {newUserRole === r.id && <div className="w-2.5 h-2.5 rounded-full bg-white shadow-sm" />}
                            </div>
                            <div>
                              <div className={`font-black tracking-tight text-base mb-1 ${newUserRole === r.id ? `text-white` : 'text-slate-300'}`}>{r.label}</div>
                              <div className={`text-xs font-medium ${newUserRole === r.id ? `text-indigo-300` : 'text-slate-500'}`}>{r.desc}</div>
                            </div>
                          </div>
                        </label>
                      ))}
            ...
            >
              <div className="glass-card !bg-transparent !p-0 !rounded-[2.5rem] overflow-hidden border border-[var(--crm-border)] shadow-2xl">
                <div className="p-6 sm:px-10 border-b border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white/5">
                  ...
                  <div className="text-[10px] w-fit font-black text-indigo-300 bg-indigo-500/20 px-3 py-1.5 rounded-lg border border-indigo-500/30 uppercase tracking-widest shadow-sm">{teamMembers.length} Members</div>
                </div>

                {loading ? (
                  <div className="w-full">
                    {[...Array(5)].map((_, i) => (
                      <div key={i} className="p-6 border-b border-white/10 flex items-center justify-between animate-pulse bg-black/20">
                        ...
                        <div className="hidden sm:block w-24 h-6 bg-white/10 rounded-lg"></div>
                  ...
                  <div className="p-24 sm:p-32 text-center flex flex-col items-center gap-6 bg-black/20">
                    <div className="w-20 h-20 bg-white/5 border border-white/10 rounded-3xl flex items-center justify-center text-slate-500 shadow-inner">
                      <Users size={40} />
                    </div>
                      ...
                      <tbody className="divide-y divide-white/5 bg-black/20">
                        {teamMembers.map((member, idx) => (
                          ...
                          >
                            <td className="p-6">
                              <div className="flex items-center gap-4">
                                <div className={`relative shrink-0 w-12 h-12 rounded-xl flex items-center justify-center font-black text-lg border-2 shadow-sm transition-transform group-hover:scale-110 ${member.role === 'admin' ? 'bg-indigo-500/20 text-indigo-400 border-indigo-500/30' : 'bg-black/40 text-slate-300 border-white/10'}`}>
                            ...
                            <td className="p-6 hidden lg:table-cell">
                              <div className="flex flex-col gap-1">
                                <span className={`w-fit px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-widest border ${member.active !== false ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border-rose-500/20'}`}>
                                  {member.active !== false ? 'Active' : 'Inactive'}
                                </span>
                                <span className="text-[10px] font-bold text-slate-500">{member.createdAt?.toDate ? member.createdAt.toDate().toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' }) : 'Legacy'}</span>
                              </div>
                            </td>
                            <td className="p-6 text-right">
                              <div className="flex items-center justify-end gap-3">
                                <button
                                  ...
                                  className="p-2.5 rounded-xl text-slate-400 hover:text-indigo-400 hover:bg-white/10 border border-transparent transition-all shadow-sm active:scale-95"
                                >
                                  <Edit size={18} />
                                </button>
                                {(role === 'admin' || role === 'super_admin') && member.uid !== user.uid && (
                                  <button
                                    ...
                                    className="p-2.5 rounded-xl text-slate-400 hover:text-rose-400 hover:bg-white/10 border border-transparent transition-all shadow-sm active:scale-95"
                                  >
                                    <Trash2 size={18} />
                                  </button>
                                )}
