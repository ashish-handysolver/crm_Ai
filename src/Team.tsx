                    ...
                    <div>
                      ...
                      <div className="space-y-4">
                        {[
                          ...
                          <label key={r.id} className={`group border-2 rounded-2xl p-5 cursor-pointer flex items-start gap-4 transition-all duration-300 shadow-sm hover:shadow-md ${role === r.id ? `border-${r.color}-500 bg-${r.color}-50 shadow-${r.color}-500/10` : 'border-orange-100 hover:border-orange-200'}`}>
                            <input type="radio" name="role" value={r.id} checked={role === r.id} onChange={() => setRole(r.id)} className="hidden" />
                            <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 mt-1 transition-all ${role === r.id ? `border-${r.color}-500 bg-${r.color}-500` : 'border-orange-300'}`}>
                              {role === r.id && <div className="w-2.5 h-2.5 rounded-full bg-orange-50 shadow-sm" />}
                            </div>
                            <div>
                    ...
                    {teamMembers.map((member, idx) => (
                      ...
                      >
                        <div className="flex items-center gap-6 w-full sm:w-auto mb-4 sm:mb-0">
                          <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br flex items-center justify-center font-black text-2xl border transition-all duration-500 shadow-sm group-hover:shadow-md ${member.role === 'admin' ? 'from-orange-100 to-purple-100 text-orange-700 border-orange-200/50 group-hover:scale-105' : 'from-slate-100 to-slate-200 text-slate-600 border-orange-200/50'}`}>
                            {member.displayName?.charAt(0) || <User size={24} />}
                          </div>
                          <div className="overflow-hidden min-w-0 flex-1">
                            <div className="font-extrabold text-black text-lg flex items-center gap-3">
                              <span className="truncate">{member.displayName}</span>
                              <div className="flex gap-1.5 shrink-0">
                                {member.role === 'admin' && (
                                  <span className="text-[9px] font-black bg-orange-500 text-white px-2 py-0.5 rounded shadow-sm uppercase tracking-tighter">Nexus Admin</span>
                                )}
                                {member.uid === user.uid && (
                        ...
                        <div className="flex flex-col items-start sm:items-end gap-1.5 shrink-0 pl-[88px] sm:pl-0">
                          ...
                          <div className="text-xs font-bold text-slate-500 bg-orange-50 px-3 py-1.5 rounded-lg border border-orange-100 shadow-sm group-hover:border-orange-100 group-hover:text-orange-600 transition-colors">
