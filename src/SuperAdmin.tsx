...
export default function SuperAdmin() {
  ...
  return (
    <div className="flex-1 bg-slate-50 text-slate-900 p-4 sm:p-6 lg:p-10 min-h-[100dvh] font-sans">
        ...
        <AnimatePresence mode="wait">
          {!selectedOrg ? (
            ...
            >
              <header className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                  ...
                  <h1 className="text-3xl md:text-5xl font-black tracking-tight text-slate-900 mb-2">Super Admin Console</h1>
                ...
                <button 
                  ...
                  className="px-6 py-3 bg-white border border-slate-200 rounded-2xl text-sm font-bold text-slate-600 hover:text-rose-600 hover:border-rose-100 transition-all shadow-sm"
                >
                  Terminate Session
                </button>
              </header>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
                {[
                  ...
                  { label: 'Registered Users', value: allUsers.length, icon: Users, color: 'text-indigo-500', bg: 'bg-indigo-50' },
                  ...
                  <motion.div key={idx} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: idx * 0.05 }} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex items-center gap-4">
              ...
              <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden flex flex-col">
                <div className="p-6 border-b border-slate-50 flex flex-col md:flex-row justify-between items-center gap-6">
                  <div className="flex p-1 bg-slate-100 rounded-2xl w-full md:w-auto">
                    <button onClick={() => setActiveTab('companies')} className={`flex-1 md:flex-none px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${activeTab === 'companies' ? 'bg-white text-slate-900 shadow-md' : 'text-slate-500 hover:text-slate-700'}`}>Organizations</button>
                    <button onClick={() => setActiveTab('users')} className={`flex-1 md:flex-none px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${activeTab === 'users' ? 'bg-white text-slate-900 shadow-md' : 'text-slate-500 hover:text-slate-700'}`}>Users</button>
                    <button onClick={() => setActiveTab('leads')} className={`flex-1 md:flex-none px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${activeTab === 'leads' ? 'bg-white text-slate-900 shadow-md' : 'text-slate-500 hover:text-slate-700'}`}>Leads</button>
                  </div>
                  ...
                  <div className="relative w-full md:w-96 group">
                    ...
                    <input 
                      ...
                      className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 font-semibold outline-none transition-all shadow-inner"
                    />
                  </div>
                </div>
                    ...
                    <table className="w-full text-left">
                      ...
                      <tbody className="divide-y divide-slate-50">
                        {filteredCompanies.map(c => (
                          <tr key={c.id} className="hover:bg-slate-50/50 transition-all group cursor-pointer" onClick={() => fetchOrgDetails(c)}>
                            <td className="py-5 px-8">
                              <div className="flex items-center gap-4">
                                <div className="w-10 h-10 bg-indigo-50 text-indigo-600 rounded-xl flex items-center justify-center font-black shadow-sm group-hover:scale-110 transition-transform">{c.name?.charAt(0)}</div>
                                <div className="font-extrabold text-slate-900 text-base">{c.name}</div>
                              </div>
                            </td>
                            <td className="py-5 px-6"><code className="text-xs bg-slate-100 px-2 py-1 rounded-lg text-slate-600 font-bold">{c.id}</code></td>
                            ...
                            <td className="py-5 px-8 text-right">
                              <button className="p-2 text-slate-400 hover:text-indigo-600 bg-slate-50 hover:bg-white border border-transparent hover:border-indigo-100 rounded-xl transition-all shadow-sm"><ArrowUpRight size={18} /></button>
                            </td>
                          </tr>
                        ))}
                      </tbody>
                        ...
                        {filteredLeads.map(l => {
                          ...
                          return (
                            <tr key={l.id} className="hover:bg-slate-50/50 transition-all group">
                              ...
                              <td className="py-5 px-6"><span className="text-[10px] bg-slate-100 px-2 py-1 rounded-lg text-slate-500 font-bold">{l.source || 'Direct'}</span></td>
                              <td className="py-5 px-6">
                                <span className={`text-[10px] font-black uppercase px-2 py-1 rounded-lg border ${l.status === 'New' ? 'bg-blue-50 text-blue-600 border-blue-100' : 'bg-slate-50 text-slate-500 border-slate-100'}`}>{l.status || 'Unknown'}</span>
                              </td>
                              ...
                              <td className="py-5 px-8 text-right">
                                <div className="flex items-center justify-end gap-2">
                                  <button 
                                    ...
                                    className="p-2 text-slate-400 hover:text-indigo-600 bg-slate-50 hover:bg-white rounded-xl transition-all"
                                  >
                                    <ExternalLink size={18} />
                                  </button>
                                  <button 
                                    onClick={(e) => { e.stopPropagation(); handleDeleteLead(l.id); }}
                                    className="p-2 text-slate-300 hover:text-rose-600 bg-slate-50 hover:bg-white rounded-xl transition-all"
                                  >
                                    <Trash2 size={18} />
                                  </button>
                                </div>
                      ...
                      <tbody className="divide-y divide-slate-50">
                        {filteredUsers.map(u => (
                          <tr key={u.id} className="hover:bg-slate-50/50 transition-all group">
                            <td className="py-5 px-8">
                              <div className="flex items-center gap-4">
                                <img src={u.photoURL || `https://ui-avatars.com/api/?name=${u.displayName}&background=random`} className="w-10 h-10 rounded-full object-cover border-2 border-white shadow-sm" alt="" />
                                <div>
                                  <div className="font-extrabold text-slate-900">{u.displayName}</div>
                            ...
                            <td className="py-5 px-6">
                              <span className={`text-[10px] font-black uppercase px-2 py-1 rounded-lg border ${u.role === 'super_admin' ? 'bg-pink-50 text-pink-600 border-pink-100' : u.role === 'admin' ? 'bg-indigo-50 text-indigo-600 border-indigo-100' : 'bg-slate-50 text-slate-500 border-slate-100'}`}>{u.role || 'user'}</span>
                            </td>
            ...
            <motion.div 
              ...
              exit={{ opacity: 0, x: -20 }}
            >
              <button onClick={() => setSelectedOrg(null)} className="flex items-center gap-2 text-slate-500 hover:text-slate-900 font-bold mb-8 transition-colors group">
              ...
              <div className="flex flex-col lg:flex-row gap-10 items-start">
                ...
                <div className="w-full lg:w-80 shrink-0 space-y-6">
                  <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm text-center">
                    ...
                    <h2 className="text-4xl font-black text-slate-900 tracking-tight">{selectedOrg.name}</h2>
                    ...
                    <div className="mt-8 pt-8 border-t border-slate-50 space-y-4">
                       <div className="flex justify-between items-center text-sm">
                          ...
                          <span className="text-slate-900 font-black">{orgData.leads.length}</span>
                       </div>
                       <div className="flex justify-between items-center text-sm">
                          <span className="text-slate-400 font-bold uppercase tracking-tight text-[10px]">Active Users</span>
                          <span className="text-slate-900 font-black">{orgData.users.length}</span>
                       </div>
                    </div>
                  </div>

                  <div className="bg-white/50 p-6 rounded-3xl border border-slate-100 text-sm">
                ...
                <div className="flex-1 w-full space-y-10">
                  {loadingOrg ? (
                    ...
                    <>
                      {/* Leads Section */}
                      <section>
                         <h3 className="text-xl font-black text-slate-900 mb-6 flex items-center gap-3"><Users className="text-indigo-400" /> Organizational Leads</h3>
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {orgData.leads.length > 0 ? orgData.leads.map(lead => (
                              <div key={lead.id} className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm flex items-center justify-between group hover:border-indigo-100 transition-all">
                                 <div className="flex items-center gap-4">
                                    ...
                                    <div className="cursor-pointer" onClick={() => setSelectedLead(lead)}>
                                       <h4 className="font-extrabold text-slate-900 group-hover:text-indigo-600 transition-colors">{lead.name}</h4>
                                       <p className="text-xs text-slate-400 font-bold">{lead.email || lead.phone || 'No Contact Data'}</p>
                                    </div>
                                 </div>
                                <span className={`text-[9px] font-black uppercase px-2 py-1 rounded-lg ${lead.status === 'New' ? 'bg-blue-50 text-blue-600' : 'bg-slate-50 text-slate-400'}`}>
                                   {lead.status}
                                </span>
                              </div>
                            )) : (
                              <div className="col-span-full py-10 text-center text-slate-400 font-bold border-2 border-dashed border-slate-200 rounded-3xl uppercase tracking-widest text-xs">No Leads Recorded</div>
                            )}
                         </div>
                      </section>
                      ...
                      <section>
                         <div className="flex items-center justify-between mb-6">
                            <h3 className="text-xl font-black text-slate-900 flex items-center gap-3"><ShieldCheck className="text-rose-400" /> Account Access</h3>
                            <button 
                         ...
                         <AnimatePresence>
                            {isAddingUser && (
                               <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="mb-6 bg-slate-100 rounded-2xl p-6 border border-slate-200 overflow-hidden">
                                  <form onSubmit={handleAddUserToOrg} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                     <input type="text" placeholder="Full Name" required value={newUserDisplayName} onChange={e => setNewUserDisplayName(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm font-bold" />
                                     <input type="email" placeholder="Email Address" required value={newUserEmail} onChange={e => setNewUserEmail(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm font-bold" />
                                     <input type="password" placeholder="Password" required minLength={6} value={newUserPassword} onChange={e => setNewUserPassword(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm font-bold" />
                                     <select value={newUserRole} onChange={e => setNewUserRole(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm font-bold">
                                        <option value="user">Standard User</option>
                                        <option value="admin">Org Admin</option>
                                     </select>
                                     <button type="submit" disabled={actionLoading} className="md:col-span-2 bg-blue-600 text-white py-3 rounded-xl font-bold text-sm hover:bg-blue-700 disabled:opacity-50">
                      ...
                      <section>
                         <h3 className="text-xl font-black text-slate-900 mb-6 flex items-center gap-3"><Clock className="text-emerald-400" /> Recent Intelligence Logs</h3>
                         <div className="space-y-4">
                            {orgData.recentActivities.length > 0 ? orgData.recentActivities.map(rec => (
                              <div key={rec.id} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
                                <div className="flex items-center gap-4">
                                ...
                                <code className="text-[10px] bg-slate-50 text-slate-400 px-3 py-1.5 rounded-xl font-bold">{rec.id}</code>
                              </div>
                            )) : (
                              <div className="py-10 text-center text-slate-400 font-bold border-2 border-dashed border-slate-200 rounded-3xl uppercase tracking-widest text-xs">No Recent Activity Found</div>
                            )}
                         </div>
                      </section>
                    </>
                ...
                <div className="p-8">
                  <div className="flex justify-between items-start mb-8">
                    <div className="flex items-center gap-5">
                      <div className="w-16 h-16 bg-indigo-50 text-indigo-600 rounded-2xl flex items-center justify-center text-2xl font-black shadow-inner">
                        {selectedLead.name?.charAt(0)}
                      </div>
                      <div>
                        <h2 className="text-3xl font-black text-slate-900 leading-tight">{selectedLead.name}</h2>
                        <div className="flex items-center gap-3 mt-1">
                          <span className="px-2.5 py-1 bg-slate-100 text-slate-500 rounded-lg text-[10px] font-black uppercase tracking-widest">{selectedLead.status || 'New'}</span>
                          <span className="text-slate-400 text-xs font-bold">{selectedLead.id}</span>
                        </div>
                      </div>
                    </div>
                    <button onClick={() => setSelectedLead(null)} className="p-2 hover:bg-slate-50 rounded-xl transition-colors text-slate-400 hover:text-slate-900">
                      <ArrowLeft size={24} />
                    </button>
                  </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center gap-3 text-slate-700 font-bold">
                            <Mail size={16} className="text-indigo-400" /> {selectedLead.email || 'N/A'}
                          </div>
                          <div className="flex items-center gap-3 text-slate-700 font-bold">
                            <Globe size={16} className="text-indigo-400" /> {selectedLead.phone || 'N/A'}
                          </div>
                        </div>
                      </div>
                      <div>
                  ...
                  <div className="flex gap-4">
                    <button className="flex-1 bg-indigo-600 text-white py-4 rounded-2xl font-black text-sm shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all flex items-center justify-center gap-2">
                       <Mail size={18} /> Contact Lead
                    </button>
                    <button onClick={() => setSelectedLead(null)} className="px-8 bg-slate-100 text-slate-600 py-4 rounded-2xl font-black text-sm hover:bg-slate-200 transition-all">
                       Close
                    </button>
                    <button 
