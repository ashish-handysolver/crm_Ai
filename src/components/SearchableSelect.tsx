  ...
  return (
    <div className={`relative w-full ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`} ref={containerRef}>
      ...
      <div 
        ...
        className={`w-full bg-[var(--crm-input-bg)] border border-[var(--crm-border)] rounded-2xl transition-all text-[var(--crm-text)] shadow-inner cursor-pointer flex items-center justify-between hover:border-indigo-500/50 ${compact ? 'px-4 py-2.5 text-sm' : 'px-5 py-4 text-base'} ${isOpen ? 'ring-4 ring-indigo-500/20 border-indigo-500 bg-[var(--crm-bg)]/60' : ''} ${disabled ? 'pointer-events-none' : ''}`}
      >
      ...
      <AnimatePresence>
        {isOpen && !disabled && (
          <motion.div
            ...
            className="absolute z-[100] w-full mt-3 bg-[var(--crm-sidebar-bg)] border border-[var(--crm-border)] rounded-[2rem] shadow-2xl overflow-hidden backdrop-blur-2xl"
          >
            {!hideSearch && (
              <div className="p-4 border-b border-[var(--crm-border)] flex items-center gap-3 bg-[var(--crm-bg)]/30">
            ...
            <div className="max-h-[300px] overflow-y-auto hide-scrollbar p-2">
              {onAddNew && (
                <div 
                  ...
                  className="mb-2 p-3.5 bg-indigo-500/5 hover:bg-indigo-500/15 text-indigo-400 rounded-2xl cursor-pointer flex items-center gap-4 border border-indigo-500/20 transition-all group"
                >
              ...
              {filteredOptions.length > 0 ? (
                filteredOptions.map((option) => (
                  <div
                    ...
                    className={`flex items-center justify-between px-4 py-3.5 rounded-2xl cursor-pointer transition-all mb-1 ${value === option.id ? 'bg-indigo-500/15 text-indigo-400 border border-indigo-500/20' : 'hover:bg-indigo-500/5 text-[var(--crm-text)] border border-transparent hover:border-indigo-500/10'}`}
                  >
                    <div className="flex flex-col">
