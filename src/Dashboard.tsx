...
import { 
  Plus, TrendingUp, Zap, Target, MoreHorizontal, Activity, ArrowUpRight, Users, Sparkles, UserCircle, CalendarDays
} from 'lucide-react';
...
export default function Dashboard({ user }: { user: any }) {
  ...
  const pipelineData = [
    { name: 'DISCOVERY', value: getPhaseCount('DISCOVERY'), color: 'bg-blue-500' },
    { name: 'DEMO', value: getPhaseCount('DEMO'), color: 'bg-indigo-500' },
    { name: 'NEGOTIATION', value: getPhaseCount('NEGOTIATION'), color: 'bg-violet-500' },
    { name: 'ONBOARDING', value: getPhaseCount('ONBOARDING'), color: 'bg-emerald-500' },
  ];

  // Logic for conversion: onboarding / total
  const conversionRate = totalClients > 0 ? ((getPhaseCount('ONBOARDING') / totalClients) * 100).toFixed(1) : '0';
              ...
              <motion.div key="overview" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="bg-white rounded-[2rem] p-8 border border-slate-100 shadow-sm relative overflow-hidden group">
                    ...
                    <div className="text-xs text-slate-400 mt-2 font-bold">Leads in Onboarding</div>
                  </div>
                  <div className="bg-white rounded-[2rem] p-8 border border-slate-100 shadow-sm relative overflow-hidden group">
                    <div className="text-[10px] font-bold text-slate-400 tracking-widest uppercase flex justify-between">
                ...
                <section className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
                  <h2 className="text-xl font-extrabold mb-6">Sales Stages</h2>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {pipelineData.map(item => (
                      <div key={item.name} className="bg-slate-50 p-6 rounded-2xl border border-slate-100">
                        <div className="text-[10px] font-bold text-slate-400 uppercase mb-2">{item.name}</div>
                      ...
                      <tbody>
                        {recentLeads.map(item => (
                          <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                            ...
                            <td className="py-4 px-8">
                              <span className={`text-[10px] font-bold px-2 py-1 rounded-full ${item.score > 70 ? 'text-emerald-700 bg-emerald-100' : item.score > 40 ? 'text-amber-700 bg-amber-100' : 'text-red-700 bg-red-100'}`}>
                                 {item.score > 70 ? 'Hot' : item.score > 40 ? 'Warm' : 'Cold'}
                              </span>
                            </td>
