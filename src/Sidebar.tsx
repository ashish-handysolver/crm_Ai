...
function NavItem({ to, icon, label, colorClass, hoverBgClass, onClick }: { to: string, icon: React.ReactElement, label: string, colorClass: string, hoverBgClass: string, onClick?: () => void }) {
  ...
  return (
    ...
    >
      {isActive && (
        <motion.div layoutId="sidebar-active" className="absolute inset-0 bg-orange-50/10 rounded-2xl border border-orange-50/10" />
      )}
...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      {mobileOverlay}
      <aside
        className={`fixed lg:sticky top-0 left-0 h-[100dvh] w-[280px] bg-[#0A0D14] border-r border-orange-50/5 shadow-2xl lg:shadow-none flex flex-col z-50 transform transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] lg:translate-x-0 overflow-hidden ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        {/* Glow Effects */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-64 h-64 bg-orange-500/10 blur-[80px] rounded-full pointer-events-none"></div>
        ...
        <div className="px-6 pt-8 pb-6 flex items-center justify-between relative z-10">
          <Link to="/" onClick={onClose} className="flex items-center gap-3 group">
            <div className="w-12 h-12 bg-orange-50 rounded-xl flex items-center justify-center shadow-lg shadow-slate-200/50 border border-orange-100 p-1.5 overflow-hidden">
              <img src="/logo.png" alt="Handysolver Logo" className="w-full h-full object-contain" />
            </div>
            <span className="text-2xl font-black tracking-tight bg-clip-text bg-gradient-to-r from-slate-900 to-slate-700" style={{ color: '#a5aab3' }}>Handysolver</span>
          </Link>

          <button onClick={onClose} className="lg:hidden p-2 text-slate-400 hover:text-white bg-orange-50/5 rounded-full backdrop-blur transition-colors">
            <X size={20} />
          </button>
        </div>
        ...
        <nav className="flex-1 px-4 py-4 space-y-1.5 overflow-y-auto scrollbar-hide relative z-10">
          ...
          <NavItem onClick={onClose} to="/clients" icon={<Users />} label="Clients" colorClass="text-orange-400" hoverBgClass="" />
