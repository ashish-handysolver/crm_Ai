...
export default function Reports({ user }: { user: any }) {
  ...
  const visibleRecordings = role === 'team_member'
    ? recordings.filter((rec: any) => rec.authorUid === user.uid || leads.some((lead: any) => lead.id === rec.leadId))
    : recordings;

  const enrichedRecordings = visibleRecordings
    .map(rec => {
